...
| 图层 | 提供图层工作台，可选中、重命名、显示/隐藏、锁定图层，并支持上移或下移层级。 |
| 布局 | 支持以画布或选区为参照进行六向对齐、水平/垂直等距分布，以及批量统一宽度或高度。 |
| 视图 | 支持滚轮与按钮缩放、空格/Alt 拖拽平移、聚焦选区和一键重置视图。 |
...
新建设计后，可以先在右侧设置画布尺寸，再从左侧素材库添加图标、添加文字或拖入图片。选中对象后，右侧属性面板可调整尺寸、X/Y 坐标、旋转角度、透明度和颜色。右侧底部的图层工作台可用于重命名、隐藏、锁定和调整对象层级；多选对象后，可使用“对齐与分布”工具快速完成排版。画布中存在对象时，“保存”会下载项目文件；使用“打开”即可恢复该设计。
